...
class BaseBarAggregator(ABC):
    ...
    def __init__(self, instrument_id: str, bar_type: BarType) -> None:
    ...
    def _update_bar(self, trade: Trade) -> None:
        ...
        self.current_bar.volume += trade.quantity
        self.current_bar.ticks_count += 1
        self.current_bar.dollar_volume += trade.price * trade.quantity
...
class TimeBarAggregator(BaseBarAggregator):
    ...
    def __init__(self, instrument_id: str, interval: timedelta) -> None:
        super().__init__(instrument_id, BarType.TIME)
...
class VolumeBarAggregator(BaseBarAggregator):
    ...
    def __init__(self, instrument_id: str, volume_threshold: float) -> None:
        super().__init__(instrument_id, BarType.VOLUME)
...
class DollarBarAggregator(BaseBarAggregator):
    ...
    def __init__(self, instrument_id: str, dollar_threshold: float) -> None:
        super().__init__(instrument_id, BarType.DOLLAR)
...
class BarFactory:
    ...
    def create_aggregator(
        bar_type: Literal[BarType.TIME], instrument_id: str, *, interval: timedelta
    ) -> TimeBarAggregator: ...

    @overload
    @staticmethod
    def create_aggregator(
        bar_type: Literal[BarType.VOLUME], instrument_id: str, *, threshold: float
    ) -> VolumeBarAggregator: ...

    @overload
    @staticmethod
    def create_aggregator(
        bar_type: Literal[BarType.DOLLAR], instrument_id: str, *, threshold: float
    ) -> DollarBarAggregator: ...

    @staticmethod
    def create_aggregator(
        bar_type: BarType, instrument_id: str, **kwargs: Any
    ) -> BaseBarAggregator:

        if bar_type == BarType.TIME:
            if "interval" not in kwargs:
                raise ValueError("Time bars require an 'interval' (timedelta).")
            return TimeBarAggregator(instrument_id, kwargs["interval"])
        elif bar_type == BarType.VOLUME:
            if "threshold" not in kwargs:
                raise ValueError("Volume bars require a 'threshold' (float).")
            return VolumeBarAggregator(instrument_id, kwargs["threshold"])
        elif bar_type == BarType.DOLLAR:
            if "threshold" not in kwargs:
                raise ValueError("Dollar bars require a 'threshold' (float).")
            return DollarBarAggregator(instrument_id, kwargs["threshold"])
        else:
