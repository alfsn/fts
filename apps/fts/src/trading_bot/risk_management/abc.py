...
from quant_core.enums import SizingStrategyType
...
class BaseSizingStrategy(ABC):
    ...
    def calculate_size(self, input_data: SizingInput) -> SizingOutput:
        ...
        :return: A SizingOutput object specifying the amount_quote
                 ...
                 SizingOutput with amount_quote=0 and quantity_shares=0.
        """
        pass
    ...
    def _get_execution_price(self, input_data: SizingInput) -> float:
        ...
        if book is None or (not book.bids and not book.asks):
            if market_data.recent_bars:
                ...
                logger.debug(
                    f"No order book for {signal.instrument_id}. "
                    f"Falling back to last bar close price: {price}"
                )
                return price
            else:
                logger.warning(
                    f"No order book or historical bars available for {signal.instrument_id}."
                )
                return 0.0
        ...
        try:
            if signal.signal_type == SignalType.BUY:
                if not book.asks:
                    logger.warning(f"No asks available for {signal.instrument_id}.")
                    return 0.0
                return book.asks[0].price
            else:  # SignalType.SELL
                if not book.bids:
                    logger.warning(f"No bids available for {signal.instrument_id}.")
                    return 0.0
                return book.bids[0].price
        except IndexError:
            logger.error(
                f"Order book was empty for {signal.instrument_id} despite check."
            )
            return 0.0
