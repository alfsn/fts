...
class KellyCriterionSizer(BaseSizingStrategy):
    ...
    def calculate_size(self, input_data: SizingInput) -> SizingOutput:
        ...
        if book is None:
            logger.warning(
                f"No order book available for {input_data.signal.instrument_id}, "
        ...
        try:
            if input_data.signal.signal_type == SignalType.BUY:
                if not book.asks:
                    logger.warning(
                        f"No asks available for {input_data.signal.instrument_id}, "
                        "cannot calculate Kelly size."
                    )
                    return SizingOutput(amount_quote=0, quantity_shares=0)
                ...
                if not book.bids:
                    logger.warning(
                        f"No bids available for {input_data.signal.instrument_id}, "
                        "cannot calculate Kelly size."
                    )
                    return SizingOutput(amount_quote=0, quantity_shares=0)
            ...
            logger.error(
                f"Error calculating Kelly odds for {input_data.signal.instrument_id}: {e}"
            )
            return SizingOutput(amount_quote=0, quantity_shares=0)
        ...
        if f_kelly <= 0:
            logger.info(
                f"No edge found for {input_data.signal.instrument_id}. "
