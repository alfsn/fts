...
class XGBoostTrainer(BaseONNXModelTrainer):
    ...
    def __init__(
        ...
        self.config = config or XGBoostConfig(lookback_period=lookback_period)
        self.lookback_period = self.config.lookback_period
        from trading_bot.core.transforms import LogReturnTransform
    ...
    def _train_to_onnx(self, historical_bars: Sequence[BarData]) -> Any:
        ...
        model = xgb.XGBRegressor(
            ...
            max_depth=getattr(self.config, "max_depth", 3),
            learning_rate=getattr(self.config, "learning_rate", 0.05),
            subsample=getattr(self.config, "subsample", 1.0),
            colsample_bytree=getattr(self.config, "colsample_bytree", 1.0),
            early_stopping_rounds=getattr(self.config, "early_stopping_rounds", 10),
            reg_alpha=getattr(self.config, "reg_alpha", 0.0),
            reg_lambda=getattr(self.config, "reg_lambda", 1.0),
