...
class XGBoostConfig(BaseTrainerConfig):
    """Configuration for XGBoost Regressor model training."""

    n_estimators: int = Field(default=100, ge=1, description="Number of boosting trees")
    max_depth: int = Field(
        default=3, ge=1, description="Maximum tree depth for base learners"
    )
    learning_rate: float = Field(
        default=0.05, gt=0.0, description="Boosting learning rate (eta)"
    )
    subsample: float = Field(
        default=1.0, gt=0.0, le=1.0, description="Subsample ratio of training instances"
    )
    colsample_bytree: float = Field(
        default=1.0, gt=0.0, le=1.0, description="Subsample ratio of columns per tree"
    )
    early_stopping_rounds: int = Field(
        default=10, ge=1, description="Validation early stopping rounds"
    )
    ...
    gamma: float = Field(
        default=0.0, ge=0.0, description="Minimum loss reduction required for split"
    )
