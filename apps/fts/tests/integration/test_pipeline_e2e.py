...
def test_pipeline_e2e_flow_buy_and_sell_with_db(db_session: Session):
    ...
    order_book = OrderBook(
        bids=[PriceLevel(price=49800.0, quantity=2.0)],
        asks=[PriceLevel(price=50200.0, quantity=2.0)],
        timestamp=datetime.now(timezone.utc),
    )
    ...
    market_provider = FakeMarketProvider(data={instrument_id: market_data})
    ...
    initial_state = portfolio.get_state({instrument_id: market_data})
    assert initial_state.cash_balances[0].available == 10000.0
    ...
    post_buy_state = portfolio.get_state({instrument_id: market_data})
    assert len(post_buy_state.positions) == 1
    buy_pos = post_buy_state.positions[0]
    assert buy_pos.instrument_id == instrument_id
    assert buy_pos.quantity == pytest.approx(0.1)
    assert buy_pos.cost_basis == 50200.0
    ...
    assert db_pos.instrument_id == instrument_id
    assert db_pos.quantity == pytest.approx(0.1)
    assert db_pos.cost_basis == 50200.0
    ...
    order_book.bids = [PriceLevel(price=52000.0, quantity=2.0)]
    order_book.asks = [PriceLevel(price=52500.0, quantity=2.0)]
    ...
    post_sell_state = portfolio.get_state({instrument_id: market_data})
