...
def test_position_repository_create_update_delete(in_memory_db):
    ...
    assert open_positions[0].instrument_id == "AAPL"
    assert open_positions[0].outcome is None
    assert open_positions[0].quantity == 10.0
    assert open_positions[0].cost_basis == 150.0

    # 2. Update Position Size and Entry Price
    pos.quantity = 15.0
    pos.cost_basis = 155.0
    pos_repo.save_position(pos)

    open_positions = pos_repo.get_open_positions()
    assert len(open_positions) == 1
    assert open_positions[0].quantity == 15.0
    assert open_positions[0].cost_basis == 155.0
...
def test_order_repository_flow(in_memory_db):
    ...
    assert db_order.instrument_id == "AAPL"
    assert db_order.side == OrderSide.BUY
    assert db_order.outcome is None
    assert db_order.requested_quantity == 100.0
    ...
    assert db_order.filled_quantity == 100.0
...
def test_market_data_repository(in_memory_db):
    ...
    assert market.instrument_id == "MSFT"
    ...
    db_bars = in_memory_db.query(BarDataLogModel).filter_by(instrument_id="MSFT").all()
